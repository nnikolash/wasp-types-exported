...
BUILD_TAGS = rocksdb
BUILD_LD_FLAGS = "-X=github.com/nnikolash/wasp-types-exported/components/app.Version=$(GIT_REF_TAG)"
...
test-short:
	go test -tags $(BUILD_TAGS) -ldflags $(BUILD_LD_FLAGS) --short --count 1 -failfast $(shell go list ./... | grep -v github.com/nnikolash/wasp-types-exported/contracts/wasm)
